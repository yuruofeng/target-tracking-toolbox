function varargout = demo_single_dbt(varargin)
% DEMO_SINGLE_DBT  Detect-Before-Track algorithm demonstration.
%   Demonstrates all four DBT filters (EKF, UKF, CKF, PF) on a
%   constant turn-rate target tracking scenario with radar measurements.
%
%   Usage:
%       demo_single_dbt()                    % Run with default settings
%       demo_single_dbt('mcRuns', 10)        % Run 10 Monte Carlo iterations
%       demo_single_dbt('numSteps', 200)     % Use 200 time steps
%       demo_single_dbt('saveResults', true) % Save results to file
%
...
%
%   Example:
%       results = demo_single_dbt('mcRuns', 5, 'numSteps', 150);
%
%   Algorithms:
...
%
%   See also: tracking.single.dbt.Config, tracking.single.dbt.EKF, tracking.single.dbt.UKF, tracking.single.dbt.CKF, tracking.single.dbt.ParticleFilter
    ...
    addParameter(p, 'saveFile', fullfile(repoRoot, 'results', 'dbt_results.mat'), @ischar);
    addParameter(p, 'showPlots', true, @islogical);
    parse(p, varargin{:});
    opts = p.Results;

    cfg = tracking.single.dbt.Config('numSteps', opts.numSteps, 'dt', opts.dt, ...
    ...
    for iMc = 1:opts.mcRuns
        ...
        ukf = tracking.single.dbt.UKF(cfg);
        ckf = tracking.single.dbt.CKF(cfg);
        pf  = tracking.single.dbt.ParticleFilter(cfg);
    ...
    if opts.showPlots
        ...
        tracking.viz.Visualizer.plotComprehensiveDbt(truthX, meas, estimates, ...
            rmseStruct, labels, nK);

        tracking.viz.Visualizer.plotTimingComparison(...
