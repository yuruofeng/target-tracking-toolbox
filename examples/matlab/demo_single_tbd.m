function varargout = demo_single_tbd(varargin)
% DEMO_SINGLE_TBD  Track-Before-Detect algorithm demonstration.
%   Demonstrates DP-TBD and PF-TBD algorithms on an image-domain
%   weak target tracking scenario with low SNR.
%
%   Usage:
%       demo_single_tbd()                         % Run with default settings (CV model)
%       demo_single_tbd('motionModel', 'CT')      % Use coordinated turn model
%       demo_single_tbd('motionModel', 'Maneuver')% Use segmented maneuver trajectory
%       demo_single_tbd('numFrames', 100)         % Use 100 frames
%       demo_single_tbd('snr', 6)                 % Set SNR to 6 dB
%       demo_single_tbd('saveResults', true)      % Save results to file
%
...
%
%   Example:
%       results = demo_single_tbd('motionModel', 'Maneuver', 'numFrames', 100);
%
%   See also: tracking.single.tbd.Config, tracking.single.tbd.Scenario, tracking.single.tbd.DpTbd, tracking.single.tbd.PfTbd
    ...
    addParameter(p, 'saveFile', fullfile(repoRoot, 'results', 'tbd_results.mat'), @ischar);
    ...
    cfg = tracking.single.tbd.Config('gridSize', opts.gridSize, 'numFrames', opts.numFrames, ...
    ...
    scenario = tracking.single.tbd.Scenario(cfg);
    ...
    dp = tracking.single.tbd.DpTbd(cfg);
    ...
    pf = tracking.single.tbd.PfTbd(cfg);
    ...
    if opts.showPlots
        ...
        tracking.viz.Visualizer.plotComprehensiveTbd(trueState, dpTrack, pfState, measData, ...
            dpPosRmse, pfPosRmse, dpVelRmse, pfVelRmse, cfg);
        
        scenario.plotTrajectory(trueState, maneuverLabels);

        tracking.viz.Visualizer.plotTimingComparison([dpTime, pfTime], ...
