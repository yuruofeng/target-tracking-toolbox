function varargout = demo_single_maneuver(varargin)
% DEMO_SINGLE_MANEUVER  Maneuvering target tracking demonstration.
%   Demonstrates various algorithms for tracking maneuvering targets
%   including CV, CA, CT, Singer, CS models and IMM algorithm.
%
%   Usage:
%       demo_single_maneuver()                    % Default: compare all algorithms
%       demo_single_maneuver('algorithms', {'CV', 'IMM'})
%       demo_single_maneuver('mcRuns', 10)         % 10 Monte Carlo runs
%       demo_single_maneuver('showPlots', false)   % Disable visualization
%       demo_single_maneuver('saveResults', true)  % Save results to file
%
%   Algorithms:
...
%       results - Struct with algorithm estimates and RMSE
    ...
    cfgManeuver = tracking.single.dbt.ConfigManeuver('numSteps', opts.numSteps, 'dt', opts.dt);
    scenario = tracking.single.dbt.ScenarioManeuver(cfgManeuver);
    ...
    if opts.showPlots
        ...
        tracking.viz.Visualizer.plotManeuverComparison(truthX, meas, allEstStates, ...
...
function opts = parseArgs(args)
    ...
    exampleFolder = fileparts(mfilename('fullpath'));
    repoRoot = fileparts(fileparts(exampleFolder));
    opts.saveFile = fullfile(repoRoot, 'results', 'maneuver_results.mat');
...
function [estState, posRmse, velRmse, elapsed] = runAlgorithm(algoName, truthX, meas, cfgManeuver)
    ...
    switch algoName
        case 'CV'
            cfg = tracking.single.dbt.MotionModelConfig('CV', 'dt', dt);
            ekf = tracking.single.dbt.MotionModelEKF(cfg);
            [estState, ~, elapsed] = ekf.run(meas, x0, P0_cv);
            
        case 'CA'
            cfg = tracking.single.dbt.MotionModelConfig('CA', 'dt', dt);
            ekf = tracking.single.dbt.MotionModelEKF(cfg);
            x0_ca = [x0; 0; 0];
            [estState, ~, elapsed] = ekf.run(meas, x0_ca, P0_ca);
            
        case 'CT'
            cfg = tracking.single.dbt.MotionModelConfig('CT', 'dt', dt);
            ekf = tracking.single.dbt.MotionModelEKF(cfg);
            x0_ct = [x0; 0.01];
            [estState, ~, elapsed] = ekf.run(meas, x0_ct, P0_ct);
            
        case 'Singer'
            cfg = tracking.single.dbt.MotionModelConfig('Singer', 'dt', dt);
            ekf = tracking.single.dbt.MotionModelEKF(cfg);
            x0_ca = [x0; 0; 0];
            [estState, ~, elapsed] = ekf.run(meas, x0_ca, P0_ca);
            
        case 'CS'
            cfg = tracking.single.dbt.MotionModelConfig('CS', 'dt', dt);
            ekf = tracking.single.dbt.MotionModelEKF(cfg);
            x0_ca = [x0; 0; 0];
            [estState, ~, elapsed] = ekf.run(meas, x0_ca, P0_ca);
            
        case 'IMM'
            cfg = tracking.single.dbt.ConfigIMM('dt', dt);
            imm = tracking.single.dbt.IMM(cfg);
            x0_ca = [x0; 0; 0];
