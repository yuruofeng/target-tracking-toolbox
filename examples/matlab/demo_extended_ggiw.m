...
%
%  依赖文件:
%    tracking.extended.ggiw.GgiwFilter - GGIW滤波器类
%    tracking.extended.ggiw.generateClutter - 杂波生成函数
%    tracking.extended.ggiw.generateExtendedMeasurements - 扩展目标测量生成函数
%    tracking.extended.utils.ConfigManager - 配置管理器
%    tracking.extended.utils.Logger - 日志记录器
%
%  使用方法:
%    直接运行此脚本即可
%
...
logger = tracking.extended.utils.Logger('GGIW_Main', 'INFO');
logger.info('开始GGIW-PHD扩展目标跟踪演示');

config = tracking.extended.utils.ConfigManager.getInstance();

spatialDimension = tracking.extended.utils.Constants.SPATIAL_DIMENSION;
identityMatrix = tracking.extended.utils.Constants.getIdentityMatrix(spatialDimension);
...
surveillanceVolume = tracking.extended.utils.Constants.calculateVolume(areaBounds(1), areaBounds(2), ...
...
measurements = tracking.extended.ggiw.generateExtendedMeasurements(...
...
clutter = tracking.extended.ggiw.generateClutter(clutterRate, areaBounds, timeSteps + 1);
...
for k = 1:(timeSteps + 1)
    ...
    if mod(k, 5) == 1
        ...
        [x, y] = tracking.extended.ggiw.plotExtentEllipse(trueState(1, k), trueState(4, k), ...
        ...
        if estimates.numTargets > 0
            for t = 1:estimates.numTargets
                ...
                if size(estimates.extents, 3) >= t
                    [xEst, yEst] = tracking.extended.ggiw.plotExtentEllipse(...
