...
savePath = fullfile(repoRoot, 'results', 'phd_results.mat');
...
for iMC = 1:nMC
    scenarioMc = tracking.multi.rfs.phd.Scenario(model);
    ...
    % --- IMM-PHD 滤波
    immFilter = tracking.multi.rfs.phd.ImmPhdFilter(model);
    ...
    for k = 1:meas.K
        [ospaImm(k,1,iMC), ospaImm(k,2,iMC), ospaImm(k,3,iMC)] = ...
            tracking.metrics.OspaMetric.compute(getComps(truthMc.X{k}, [1,4]), getComps(est.IMMX{k}, [1,4]), ospaC, ospaP);
    end

    % --- SIMM-PHD 滤波
    simmFilter = tracking.multi.rfs.phd.SimmPhdFilter(model);
    ...
    for k = 1:meas.K
        [ospaSimm(k,1,iMC), ospaSimm(k,2,iMC), ospaSimm(k,3,iMC)] = ...
            tracking.metrics.OspaMetric.compute(getComps(truthMc.X{k}, [1,4]), getComps(estS.IMMX{k}, [1,4]), ospaC, ospaP);
    end
