...
%
%  依赖文件:
%    tracking.extended.starconvex.StarConvexTracker - 星凸目标跟踪器类
%    tracking.extended.starconvex.generateGroundTruth - 真实数据生成函数
%    tracking.extended.starconvex.generateMeasurements - 测量生成函数
...
logger = tracking.extended.utils.Logger('StarConvex_Main', 'INFO');
...
[groundTruthKinematic, groundTruthParams, objectBounds, sizeObject, ...
    timeSteps, deltaTime] = tracking.extended.starconvex.generateGroundTruth();
...
tracker = tracking.extended.starconvex.StarConvexTracker(numFourierCoeff, 'UKF', config);
...
for t = 1:timeSteps
    ...
    measurements = tracking.extended.starconvex.generateMeasurements(sizeObject, ...
