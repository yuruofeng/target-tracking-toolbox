...
%
%   使用方法：
%       viz = tracking.viz.Visualizer();
%       tracking.viz.Visualizer.plotDbtTrajectory(truthX, meas, estimates);
%       tracking.viz.Visualizer.plotRmseComparison(rmseData);
%       tracking.viz.Visualizer.plotTbdResults(trueState, dpTrack, pfState, measData);
%
%   色盲友好调色板 (CUD配色)：
...
%
%   See also: tracking.single.dbt.Config, tracking.single.tbd.Config
    ...
    methods (Static)
        ...
        %       fig - 图形句柄
            if nargin < 2
                width = tracking.viz.Visualizer.DEFAULT_FIG_WIDTH;
                height = tracking.viz.Visualizer.DEFAULT_FIG_HEIGHT;
            end
        ...
        %       将极坐标测量转换为笛卡尔坐标进行显示，
        %       同时显示起点和终点标记。
            tracking.viz.Visualizer.createFigure('DBT轨迹', 700, 600);

            xMeas = meas(2, :) .* cos(meas(1, :));
            yMeas = meas(2, :) .* sin(meas(1, :));

            colors = {tracking.viz.Visualizer.COLORS.blue, tracking.viz.Visualizer.COLORS.orange, ...
                      tracking.viz.Visualizer.COLORS.cyan, tracking.viz.Visualizer.COLORS.green};
            lineStyles = {'-', '--', '-.', ':'};

            hold on;
            plot(truthX(1, :), truthX(3, :), 'k-.', 'LineWidth', 1.5, 'DisplayName', '真实值');
            plot(xMeas, yMeas, 'o', 'Color', tracking.viz.Visualizer.COLORS.gray, ...
            ...
            plot(truthX(1, 1), truthX(3, 1), '^', 'Color', tracking.viz.Visualizer.COLORS.green, ...
                 'MarkerSize', 10, 'MarkerFaceColor', tracking.viz.Visualizer.COLORS.green, ...
                 'DisplayName', '起点');
            plot(truthX(1, end), truthX(3, end), 'v', 'Color', tracking.viz.Visualizer.COLORS.orange, ...
                 'MarkerSize', 10, 'MarkerFaceColor', tracking.viz.Visualizer.COLORS.orange, ...
                 'DisplayName', '终点');
        ...
        %       titleStr - (可选) 图表标题，默认为'RMSE Comparison'
            ...
            tracking.viz.Visualizer.createFigure(titleStr, 800, 500);
            colors = {tracking.viz.Visualizer.COLORS.blue, tracking.viz.Visualizer.COLORS.orange, ...
                      tracking.viz.Visualizer.COLORS.cyan, tracking.viz.Visualizer.COLORS.green};
            lineStyles = {'-', '--', '-.', ':'};
        ...
        %       titleStr - (可选) 图表标题
            ...
            tracking.viz.Visualizer.createFigure(titleStr, 600, 500);
        ...
        %       measData  - 测量数据立方体 [rows x cols x nFrames]
            ...
            colors = {tracking.viz.Visualizer.COLORS.orange, tracking.viz.Visualizer.COLORS.blue};

            imagesc(mean(measData, 3)); colormap(gca, 'parula'); hold on;

            plot(trueState(:, 2), trueState(:, 1), '-o', ...
                 'Color', tracking.viz.Visualizer.COLORS.green, 'LineWidth', 2, ...
            ...
            plot(trueState(1, 2), trueState(1, 1), 'p', ...
                 'Color', tracking.viz.Visualizer.COLORS.green, 'MarkerSize', 12, ...
                 'MarkerFaceColor', tracking.viz.Visualizer.COLORS.green, 'DisplayName', '起点');
            plot(trueState(end, 2), trueState(end, 1), 'h', ...
                 'Color', tracking.viz.Visualizer.COLORS.green, 'MarkerSize', 12, ...
                 'MarkerFaceColor', tracking.viz.Visualizer.COLORS.green, 'DisplayName', '终点');
        ...
        %       nFrames   - 帧数
            ...
            subplot(1, 2, 1);
            hold on;
            plot(1:nFrames, dpPosRmse, '-', 'Color', tracking.viz.Visualizer.COLORS.orange, ...
                 'LineWidth', 1.5, 'DisplayName', 'DP-TBD');
            plot(1:nFrames, pfPosRmse, '-', 'Color', tracking.viz.Visualizer.COLORS.blue, ...
            ...
            subplot(1, 2, 2);
            hold on;
            plot(2:nFrames, dpVelRmse, '-', 'Color', tracking.viz.Visualizer.COLORS.orange, ...
                 'LineWidth', 1.5, 'DisplayName', 'DP-TBD');
            plot(1:nFrames, pfVelRmse, '-', 'Color', tracking.viz.Visualizer.COLORS.blue, ...
                 'LineWidth', 1.5, 'DisplayName', 'PF-TBD');
        ...
        %       nFrames   - 帧数
            ...
            plot(1:nFrames, dpTrack(:, 1) - trueState(:, 1), '-', ...
                 'Color', tracking.viz.Visualizer.COLORS.orange, 'DisplayName', 'DP行');
            plot(1:nFrames, dpTrack(:, 2) - trueState(:, 2), '--', ...
                 'Color', tracking.viz.Visualizer.COLORS.orange, 'DisplayName', 'DP列');
            plot(1:nFrames, pfState(:, 1) - trueState(:, 1), '-', ...
                 'Color', tracking.viz.Visualizer.COLORS.blue, 'DisplayName', 'PF行');
            plot(1:nFrames, pfState(:, 2) - trueState(:, 2), '--', ...
                 'Color', tracking.viz.Visualizer.COLORS.blue, 'DisplayName', 'PF列');
            yline(0, 'k:');
            ...
            subplot(1, 2, 2);
            hold on;
            plot(1:nFrames, pfState(:, 5), '-', 'Color', tracking.viz.Visualizer.COLORS.blue, ...
                 'LineWidth', 1.5, 'DisplayName', 'PF估计');
            plot(1:nFrames, trueState(:, 5), '--', 'Color', tracking.viz.Visualizer.COLORS.green, ...
        ...
        %       titleStr   - (可选) 图表标题
            ...
            tracking.viz.Visualizer.createFigure(titleStr, 600, 400);
            colors = [tracking.viz.Visualizer.COLORS.blue; tracking.viz.Visualizer.COLORS.orange; ...
                      tracking.viz.Visualizer.COLORS.cyan; tracking.viz.Visualizer.COLORS.green];
        ...
        %   输出参数：
        %       fig - 图形句柄
            fig = tracking.viz.Visualizer.createFigure('DBT综合结果', 1200, 800);
            ...
            bar(barX - width/2, meanPos(1:nAlgo), width, ...
                     'FaceColor', tracking.viz.Visualizer.COLORS.blue, 'DisplayName', categories{1});
            bar(barX + width/2, meanVel(1:nAlgo), width, ...
                     'FaceColor', tracking.viz.Visualizer.COLORS.orange, 'DisplayName', categories{2});
        ...
        %   输出参数：
        %       fig - 图形句柄
            fig = tracking.viz.Visualizer.createFigure('TBD综合结果', 1400, 900);
            ...
            plot(trueState(:, 2), trueState(:, 1), '-o', ...
                 ...
                 'MarkerSize', 3, 'DisplayName', '真实值');
            if ~isempty(dpTrack)
                plot(dpTrack(:, 2), dpTrack(:, 1), '--s', 'Color', tracking.viz.Visualizer.COLORS.orange, ...
                     'LineWidth', 1.5, 'MarkerSize', 4, 'DisplayName', 'DP-TBD');
            end
            if ~isempty(pfState)
                plot(pfState(:, 2), pfState(:, 1), '--s', 'Color', tracking.viz.Visualizer.COLORS.blue, ...
            ...
            subplot(2, 3, 2);
            hold on;
            plot(1:nF, dpPosRmse, '-', 'Color', tracking.viz.Visualizer.COLORS.orange, ...
                 'LineWidth', 1.5, 'DisplayName', 'DP-TBD');
            plot(1:nF, pfPosRmse, '-', 'Color', tracking.viz.Visualizer.COLORS.blue, ...
                 'LineWidth', 1.5, 'DisplayName', 'PF-TBD');
            xlabel('帧'); ylabel('位置RMSE [像素]');
            title('位置RMSE');
            legend('Location', 'best'); grid on; hold off;
            ...
            subplot(2, 3, 4);
            hold on;
            plot(1:nF, dpVelRmse, '-', 'Color', tracking.viz.Visualizer.COLORS.orange, ...
                 'LineWidth', 1.5, 'DisplayName', 'DP-TBD');
            plot(1:nF, pfVelRmse, '-', 'Color', tracking.viz.Visualizer.COLORS.blue, ...
                 'LineWidth', 1.5, 'DisplayName', 'PF-TBD');
            xlabel('帧'); ylabel('速度RMSE [像素/帧]');
            title('速度RMSE');
            legend('Location', 'best'); grid on; hold off;
        ...
        %   输出参数：
        %       fig - 图形句柄
            fig = tracking.viz.Visualizer.createFigure('机动目标跟踪对比', 1400, 900);
            ...
            fontSize = tracking.viz.Visualizer.DEFAULT_FONT_SIZE;
            trueLineWidth = 2.5;
            estLineWidth = 1.8;
            markerSize = 6;
