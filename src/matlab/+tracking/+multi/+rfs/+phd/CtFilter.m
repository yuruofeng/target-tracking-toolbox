...
%
%   See also: tracking.multi.rfs.phd.CvFilter, tracking.multi.rfs.phd.CaFilter, tracking.multi.rfs.phd.MultiModelFilter
    ...
    methods (Static)
        ...
        %       PPred - 5D predicted covariance [5 x 5 x nComps]
            ...
            [~, P4] = tracking.multi.rfs.phd.MathUtils.reduceState6to4(m, P);
            P5 = zeros(5, 5, nComps);
            for ii = 1:nComps
            ...
            PPred = zeros(size(P5));
            for ii = 1:nComps
                mT = tracking.multi.rfs.phd.CtFilter.genNewState(model, m5(:, ii), 'noiseless');
                [Fekf, Gekf] = tracking.multi.rfs.phd.CtFilter.predictMat(model, m5(:, ii));
        ...
        %       PUpdate  - 5D updated covariance [5 x 5 x nComps]
            ...
            for ii = 1:nComps
                ...
                S = (S + S') / 2;
                S = tracking.multi.rfs.phd.MathUtils.ensurePositiveDefinite(S, 1e-10);
                ...
                Sfull = (Sfull + Sfull') / 2;
                Sfull = tracking.multi.rfs.phd.MathUtils.ensurePositiveDefinite(Sfull, 1e-10);
        ...
        %   Computes likelihood in 4D space (excluding omega).
            ...
            for ll = 1:nGM
                ...
                S = (S + S') / 2;
                S = tracking.multi.rfs.phd.MathUtils.ensurePositiveDefinite(S, 1e-6);
        ...
        % GATEMEAS  Ellipsoidal gating for CT model (using EKF observation Jacobian).
            ...
            validIdx = [];
            for jj = 1:nComps
                [Hekf, Uekf] = tracking.multi.rfs.phd.CtFilter.updateMat(model, m(:, jj));
                Sj = Uekf * model.R_CT * Uekf' + Hekf * P(:, :, jj) * Hekf';
                Sj = (Sj + Sj') / 2;
                Sj = tracking.multi.rfs.phd.MathUtils.ensurePositiveDefinite(Sj, 1e-10);

                Vs = chol(Sj);
                invSqrtSj = inv(Vs); %#ok<MINV>
                zPred = tracking.multi.rfs.phd.CtFilter.genObservation(model, m(:, jj), zeros(size(model.D_CT, 2), 1));
