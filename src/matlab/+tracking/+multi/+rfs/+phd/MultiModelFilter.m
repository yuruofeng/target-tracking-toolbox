...
%
%   See also: tracking.multi.rfs.phd.CvFilter, tracking.multi.rfs.phd.CtFilter, tracking.multi.rfs.phd.CaFilter, tracking.multi.rfs.phd.GaussMixture
    ...
    methods (Static)
        ...
        %       lk1~lk3 - Model likelihoods
            ...
            %% ===== Prediction =====
            % CV
            [mPred1, PPred1] = tracking.multi.rfs.phd.CvFilter.predict(model, m0{1}, P0{1});
            ...
            % CT
            [mPred2, PPred2] = tracking.multi.rfs.phd.CtFilter.predict(model, m0{2}, P0{2});
            ...
            % CA
            [mPred3, PPred3] = tracking.multi.rfs.phd.CaFilter.predict(model, m0{3}, P0{3});
            ...
            %% ===== Gating =====
            if filter.gate_flag
                Z1 = tracking.multi.rfs.phd.CvFilter.gateMeas(meas.Z{k}, filter.gamma, model, mPred1, PPred1);
                Z2 = tracking.multi.rfs.phd.CtFilter.gateMeas(meas.Z{k}, filter.gamma, model, mPred2, PPred2);
                Z3 = tracking.multi.rfs.phd.CaFilter.gateMeas(meas.Z{k}, filter.gamma, model, mPred3, PPred3);
            else
            ...
            if nMeas > 0
                % CV update
                [qz1, mT1, PT1] = tracking.multi.rfs.phd.CvFilter.update(zIntersect, model, mPred1, PPred1);
                for ell = 1:nMeas
                ...
                % CT update
                [qz2, mT2, PT2] = tracking.multi.rfs.phd.CtFilter.update(zIntersect, model, mPred2, PPred2);
                for ell = 1:nMeas
                ...
                % CA update
                [qz3, mT3, PT3] = tracking.multi.rfs.phd.CaFilter.update(zIntersect, model, mPred3, PPred3);
                for ell = 1:nMeas
            ...
            %% ===== Likelihood =====
            lk1 = tracking.multi.rfs.phd.CvFilter.computeLikelihood(model, zIntersect, mPred1, PPred1, wPred1);
            lk2 = tracking.multi.rfs.phd.CtFilter.computeLikelihood(model, zIntersect, mPred2, PPred2, wPred2);
            lk3 = tracking.multi.rfs.phd.CaFilter.computeLikelihood(model, zIntersect, mPred3, PPred3, wPred3);

            %% ===== Gaussian Mixture Management =====
            [w1, m1raw, P1raw] = tracking.multi.rfs.phd.GaussMixture.manage(w1, m1raw, P1raw, ...
                filter.elim_threshold, filter.merge_threshold, filter.L_max);
            [w2, m2raw, P2raw] = tracking.multi.rfs.phd.GaussMixture.manage(w2, m2raw, P2raw, ...
                filter.elim_threshold, filter.merge_threshold, filter.L_max);
            [w3, m3raw, P3raw] = tracking.multi.rfs.phd.GaussMixture.manage(w3, m3raw, P3raw, ...
                filter.elim_threshold, filter.merge_threshold, filter.L_max);
            L1 = length(w1);
            L2 = length(w2);
            L3 = length(w3);
            ...
            maxGMs = max([L1, L2, L3]);
            [w1, m1raw, P1raw] = tracking.multi.rfs.phd.MultiModelFilter.padComponents(w1, m1raw, P1raw, maxGMs);
            [w2, m2raw, P2raw] = tracking.multi.rfs.phd.MultiModelFilter.padComponents(w2, m2raw, P2raw, maxGMs);
            [w3, m3raw, P3raw] = tracking.multi.rfs.phd.MultiModelFilter.padComponents(w3, m3raw, P3raw, maxGMs);

            %% ===== Expand to Unified 6D =====
            % CV: 4D -> 6D
            m1 = tracking.multi.rfs.phd.MathUtils.expandState4to6(m1raw);
            P1 = tracking.multi.rfs.phd.MathUtils.expandCov4to6(P1raw);

            % CT: Remove omega to get 4D -> 6D
            m2no = m2raw([1, 2, 3, 4], :);
            P2no = P2raw(1:4, 1:4, :);
            m2 = tracking.multi.rfs.phd.MathUtils.expandState4to6(m2no);
            P2 = tracking.multi.rfs.phd.MathUtils.expandCov4to6(P2no);
