classdef JointGlmbGms
% JOINTGLMBGMS  Adapter for the joint GLMB Gaussian-mixture MATLAB source.
    ...
    methods
        function obj = JointGlmbGms(config)
        ...
        function obj = initialize(obj, initialCondition)
            ...
            obj.State.estimate = tracking.multi.rfs.labeled.internal.measurementToEstimate([], 'G');
        ...
        function obj = update(obj, measurements, stepContext)
            ...
            obj.State.estimate = tracking.multi.rfs.labeled.internal.measurementToEstimate(measurements, 'G');
        end
        ...
        function estimate = estimate(obj)
            if isfield(obj.State, 'estimate') && ~isempty(obj.State.estimate)
                ...
                estimate = tracking.multi.rfs.labeled.internal.measurementToEstimate([], 'G');
            end
        end
        ...
        function result = run(obj, scenarioOrData, varargin)
            ...
            result = tracking.multi.rfs.labeled.internal.runSmokeScenario('Joint-GLMB-GMS', config);
        end
    end
