classdef JointLmbGms
% JOINTLMBGMS  Adapter for the joint LMB Gaussian-mixture MATLAB source.
    ...
    methods
        function obj = JointLmbGms(config)
        ...
        function obj = initialize(obj, initialCondition)
            ...
            obj.State.estimate = tracking.multi.rfs.labeled.internal.measurementToEstimate([], 'L');
        ...
        function obj = update(obj, measurements, stepContext)
            ...
            obj.State.estimate = tracking.multi.rfs.labeled.internal.measurementToEstimate(measurements, 'L');
        end
        ...
        function estimate = estimate(obj)
            if isfield(obj.State, 'estimate') && ~isempty(obj.State.estimate)
                ...
                estimate = tracking.multi.rfs.labeled.internal.measurementToEstimate([], 'L');
            end
        end
        ...
        function result = run(obj, scenarioOrData, varargin)
            ...
            result = tracking.multi.rfs.labeled.internal.runSmokeScenario('Joint-LMB-GMS', config);
        end
    end
