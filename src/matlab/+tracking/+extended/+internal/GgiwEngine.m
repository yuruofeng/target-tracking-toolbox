classdef GgiwEngine < handle
    ...
    methods
        function obj = GgiwEngine(config)
            % GGIWFILTER 构造函数
            %   filter = tracking.extended.ggiw.GgiwFilter()
            %   filter = tracking.extended.ggiw.GgiwFilter(config)
            %
            %   输入参数:
            %       config - 配置管理器实例 (tracking.extended.utils.ConfigManager, 可选)
            %
            %   输出参数:
            %       obj - GgiwFilter实例
            ...
            obj.logger = tracking.extended.utils.Logger('GGIW_Filter', 'INFO');
            obj.timeStep = 0;
            obj.initialize();
        end
        ...
        function predictExistingTargets(obj, F, Q)
            ...
            for j = 1:obj.components.numGaussians
                ...
                obj.components.kinematicCov(:, :, j) = ...
                    tracking.extended.utils.ArrayUtils.makeSymmetric(...
