classdef ExtendedTargetPhdEngine < handle
    ...
    methods
        function obj = ExtendedTargetPhdEngine(config)
            % EXTENDEDTARGETPHDFILTER 构造函数
            %   filter = tracking.extended.phd.ExtendedTargetPhdFilter()
            %   filter = tracking.extended.phd.ExtendedTargetPhdFilter(config)
            %
            %   输入参数:
            ...
            obj.logger = tracking.extended.utils.Logger('ExtTargetPHD', 'INFO');
            obj.initialize();
        end
