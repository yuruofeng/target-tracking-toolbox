classdef StarConvexEngine < handle
    ...
    %
    %   使用示例:
    %       tracker = tracking.extended.starconvex.StarConvexTracker(numFourierCoeff);
    ...
    %
    %   参见: tracking.extended.utils.ConfigManager, tracking.extended.utils.Logger
    ...
    methods
        function obj = StarConvexEngine(numFourierCoeff, filterType, config)
            % STARCONVEXTRACKER 构造函数
            %   tracker = tracking.extended.starconvex.StarConvexTracker(numFourierCoeff)
            %   tracker = tracking.extended.starconvex.StarConvexTracker(numFourierCoeff, filterType)
            %   tracker = tracking.extended.starconvex.StarConvexTracker(numFourierCoeff, filterType, config)
            %
            %   输入参数:
            %       numFourierCoeff - 傅里叶系数数量 (double, 标量)
            %       filterType - 滤波器类型: 'UKF' 或 'CKF' (char, 可选, 默认 'UKF')
            %       config - 配置管理器 (tracking.extended.utils.ConfigManager, 可选)
            %
            %   输出参数:
            ...
            if isempty(config)
                obj.config = tracking.extended.utils.ConfigManager.getInstance();
            else
                obj.config = config;
            end
            
            obj.logger = tracking.extended.utils.Logger('StarConvexTracker', 'INFO');
        ...
        function predict(obj, transitionMatrix, processNoise)
            ...
            obj.covarianceMatrix = tracking.extended.utils.ArrayUtils.makeSymmetric(...
    ...
    methods (Access = private)
        function [updatedState, updatedCov] = ukfUpdate(obj, extendedState, ...
            ...
            updatedCov = tracking.extended.utils.ArrayUtils.makeSymmetric(updatedCov);
        end
