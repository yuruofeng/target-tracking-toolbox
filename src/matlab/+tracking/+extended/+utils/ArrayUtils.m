classdef ArrayUtils
    ...
    %
    %   使用示例:
    %       result = tracking.extended.utils.ArrayUtils.normalizeVector(v);
    %       cov = tracking.extended.utils.ArrayUtils.makeSymmetric(P);
    
    methods (Static)
        function normalized = normalizeVector(v)
            % NORMALIZEVECTOR 归一化向量
            %   normalized = tracking.extended.utils.ArrayUtils.normalizeVector(v)
            %
            %   输入参数:
            %       v - 输入向量 (double array)
            %
        ...
        function sym = makeSymmetric(A)
            % MAKESYMMETRIC 使矩阵对称
            %   sym = tracking.extended.utils.ArrayUtils.makeSymmetric(A)
            %
            %   输入参数:
        ...
        function result = ensurePositiveDefinite(A, minEigenvalue)
            ...
            %   result = tracking.extended.utils.ArrayUtils.ensurePositiveDefinite(A)
            %   result = tracking.extended.utils.ArrayUtils.ensurePositiveDefinite(A, minEigenvalue)
            %
            %   输入参数:
            %       A - 输入矩阵 (double matrix)
        ...
        function kronProduct = kronProduct(A, B)
            ...
            %   kronProduct = tracking.extended.utils.ArrayUtils.kronProduct(A, B)
            %
            %   输入参数:
        ...
        function indices = findNonZero(array)
            ...
            %   indices = tracking.extended.utils.ArrayUtils.findNonZero(array)
            %
            %   输入参数:
        ...
        function result = safeDivide(numerator, denominator, defaultValue)
            ...
            %   result = tracking.extended.utils.ArrayUtils.safeDivide(numerator, denominator)
            %   result = tracking.extended.utils.ArrayUtils.safeDivide(numerator, denominator, defaultValue)
            %
            %   输入参数:
            %       numerator - 分子 (double)
        ...
        function result = resampleWithReplacement(array, weights, numSamples)
            ...
            %   result = tracking.extended.utils.ArrayUtils.resampleWithReplacement(array, weights, numSamples)
            %
            %   输入参数:
            %       array - 输入数组 (double array)
