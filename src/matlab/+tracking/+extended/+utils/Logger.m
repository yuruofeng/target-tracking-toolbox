classdef Logger < handle
    ...
    %
    %   使用示例:
    %       logger = tracking.extended.utils.Logger('GGIW_PHD', 'INFO');
    %       logger.info('处理开始');
    ...
    methods
        function obj = Logger(moduleName, logLevel, logFile)
            % LOGGER 构造函数
            %   obj = tracking.extended.utils.Logger(moduleName, logLevel)
            %   obj = tracking.extended.utils.Logger(moduleName, logLevel, logFile)
            %
            %   输入参数:
            ...
            %
            %   示例:
            %       logger = tracking.extended.utils.Logger('MyModule', 'INFO');
            %       logger = tracking.extended.utils.Logger('MyModule', 'DEBUG', 'log.txt');
    ...
    methods (Static)
        function logger = getLogger(moduleName, logLevel)
            ...
            %   logger = tracking.extended.utils.Logger.getLogger('ModuleName')
            %   logger = tracking.extended.utils.Logger.getLogger('ModuleName', 'DEBUG')
            %
            %   输入参数:
            ...
            if isKey(loggers, key)
                ...
                logger = tracking.extended.utils.Logger(moduleName, logLevel);
                loggers(key) = logger;
            end
        end
    end
