classdef TrajectoryPmbmFilter
% TRAJECTORYPMBMFILTER  Adapter for extended-target trajectory PMBM code.
    ...
    methods
        function obj = TrajectoryPmbmFilter(config)
        ...
        function obj = initialize(obj, initialCondition)
            ...
            obj.State.time = 0;
            obj.State.estimate = tracking.extended.pmbm.internal.measurementToEstimate([], 'R', true);
        ...
        function obj = update(obj, measurements, stepContext)
            ...
            obj.State.estimate = tracking.extended.pmbm.internal.measurementToEstimate(measurements, 'R', true);
        end
        ...
        function estimate = estimate(obj)
            if isfield(obj.State, 'estimate') && ~isempty(obj.State.estimate)
                ...
                estimate = tracking.extended.pmbm.internal.measurementToEstimate([], 'R', true);
            end
        end
        ...
        function result = run(obj, scenarioOrData, varargin)
            ...
            result = tracking.extended.pmbm.internal.runSmokeScenario( ...
                'Extended-Target-Trajectory-PMBM', config, 'extended-target-pmbm');
        end
    end
