...
%
%   See also: tracking.association.Hungarian, tracking.multi.rfs.phd.GaussMixture
    ...
    methods (Static)
        ...
        %       cardErr - Cardinality error component
            ...
            if nargin < 3 || isempty(c)
                c = tracking.metrics.OspaMetric.DEFAULT_C;
            end
            if nargin < 4 || isempty(p)
                p = tracking.metrics.OspaMetric.DEFAULT_P;
            end
            ...
            [assignment, cost] = tracking.association.Hungarian.solve(D);
        ...
        %       cardSeq - Cardinality error sequence [1 x K]
            ...
            for k = 1:K
                [distSeq(k), locSeq(k), cardSeq(k)] = tracking.metrics.OspaMetric.compute(Xseq{k}, Yseq{k}, c, p);
            end
        end
        ...
        %       meanCard - Mean cardinality error
            ...
            [distSeq, locSeq, cardSeq] = tracking.metrics.OspaMetric.computeSeq(Xseq, Yseq, c, p);
