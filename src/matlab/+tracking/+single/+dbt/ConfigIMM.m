...
%
%   Usage:
%       cfg = tracking.single.dbt.ConfigIMM();
%       cfg = tracking.single.dbt.ConfigIMM('dt', 0.5);
%       cfg = tracking.single.dbt.ConfigIMM('models', {'CV', 'CA', 'CT'});
    ...
    methods
        ...
        %   obj = ConfigIMM('param', value, ...) sets additional parameters.
            ...
            obj.modelConfigs = {
                tracking.single.dbt.MotionModelConfig('CV', 'dt', obj.dt),
                tracking.single.dbt.MotionModelConfig('CA', 'dt', obj.dt),
                tracking.single.dbt.MotionModelConfig('CT', 'dt', obj.dt)
            };
            
            obj.models = {
                tracking.single.dbt.MotionModelEKF(obj.modelConfigs{1}),
                tracking.single.dbt.MotionModelEKF(obj.modelConfigs{2}),
                tracking.single.dbt.MotionModelEKF(obj.modelConfigs{3})
            };
    ...
    methods (Static)
        ...
        %   creates IMM configuration with specified models.
            ...
            for i = 1:nModels
                cfg.modelConfigs{i} = tracking.single.dbt.MotionModelConfig(models{i}, 'dt', dt);
                cfg.models{i} = tracking.single.dbt.MotionModelEKF(cfg.modelConfigs{i});
