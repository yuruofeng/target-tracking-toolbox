...
%
%   使用方法：
%       cfg = tracking.single.dbt.ConfigIMM();                          % 创建IMM配置
%       imm = tracking.single.dbt.IMM(cfg);                             % 创建IMM滤波器
%       [estStates, estCovars, modelProbs] = imm.run(meas, x0, P0);  % 执行滤波
%
%   模型组合示例：
%       cfg = tracking.single.dbt.ConfigIMM('modelTypes', {'CV', 'CA', 'CT'});
...
%
%   See also: tracking.single.dbt.ConfigIMM, tracking.single.dbt.MotionModelEKF, tracking.single.dbt.MotionModelConfig

    properties
        config       % IMM配置对象 (tracking.single.dbt.ConfigIMM)
    ...
    methods
        ...
        % IMM  创建IMM滤波器。
        %   obj = IMM(cfg) 使用tracking.single.dbt.ConfigIMM配置对象初始化。
        %   obj = IMM() 使用默认配置（CV+CA+CT模型）。
        %
        %   输入参数：
        %       cfg - (可选) tracking.single.dbt.ConfigIMM配置对象
            if nargin < 1
