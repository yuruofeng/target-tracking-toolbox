classdef ParticleFilter < tracking.single.dbt.BaseFilter
...
%
%   使用方法：
%       cfg = tracking.single.dbt.Config();                           % 创建配置
%       pf = tracking.single.dbt.ParticleFilter(cfg);                 % 创建粒子滤波器
...
%
%   See also: tracking.single.dbt.BaseFilter, tracking.single.dbt.EKF, tracking.single.dbt.UKF, tracking.single.dbt.CKF, tracking.single.dbt.Config
    ...
    methods
        ...
        %   obj = ParticleFilter(cfg) 使用tracking.single.dbt.Config配置对象初始化。
        %   obj = ParticleFilter() 使用默认配置。
        %
        %   输入参数：
        %       cfg - (可选) tracking.single.dbt.Config配置对象
            obj.setConfig(cfg);
    ...
    methods
        ...
        %       particlesPre - 预测后的粒子矩阵
            ...
            for i = 1:obj.numParticles
                F = tracking.models.MeasurementModel.ctDynamicMatrix(obj.config.dt, particles(:, i));
        ...
        %       particlesUpd - 粒子矩阵（保持不变）
            ...
            innov = measPre - measZ;
            innov(1, :) = tracking.core.FilterUtils.wrapToPi(innov(1, :));  % 方位角归一化
        ...
        %       particlesOut - 重采样后的粒子矩阵
            idx = tracking.core.FilterUtils.systematicResample(weights, obj.numParticles);
    ...
    methods
        ...
        %       innovCov - 新息协方差矩阵
            ...
            innov(1) = tracking.core.FilterUtils.wrapToPi(innov(1));
