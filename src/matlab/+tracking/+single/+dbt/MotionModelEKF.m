...
%
%   使用方法：
%       cfg = tracking.single.dbt.MotionModelConfig('CT');           % 创建CT模型配置
%       ekf = tracking.single.dbt.MotionModelEKF(cfg);               % 创建EKF滤波器
...
%
%   See also: tracking.single.dbt.MotionModelConfig, tracking.single.dbt.EKF, tracking.single.dbt.KalmanFilterBase, tracking.single.dbt.IMM

    properties
        config      % 配置对象 (tracking.single.dbt.MotionModelConfig)
        modelName   % 模型名称字符串
        stateDim    % 状态向量维度
        measDim     % 测量向量维度
    end
    ...
    methods
        ...
        %
        %   输入参数：
        %       cfg - (可选) tracking.single.dbt.MotionModelConfig配置对象
        %
        %   示例：
        %       ekf = tracking.single.dbt.MotionModelEKF(tracking.single.dbt.MotionModelConfig('CT'));
            if nargin < 1
    ...
    methods
        ...
        %       innovCov - 新息协方差矩阵
            ...
            innov = measZ - measPre;
            innov(1) = tracking.core.FilterUtils.wrapToPi(innov(1));  % 方位角归一化到[-pi, pi]
