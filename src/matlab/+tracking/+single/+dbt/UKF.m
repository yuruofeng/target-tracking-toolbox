classdef UKF < tracking.single.dbt.KalmanFilterBase
...
%
%   See also: tracking.single.dbt.KalmanFilterBase, tracking.single.dbt.EKF, tracking.single.dbt.CKF, tracking.single.dbt.Config
    ...
    methods
        ...
        %   obj = UKF(cfg) initializes with tracking.single.dbt.Config object.
    ...
    methods (Access = protected)
        ...
        % PREDICTSTEP  UKF prediction using sigma point transformation.
            ...
            for i = 1:nSp
                F = tracking.models.MeasurementModel.ctDynamicMatrix(obj.config.dt, spPts(:, i));
                preSp(:, i) = F * spPts(:, i);
            end
        ...
        % UPDATESTEP  UKF update using sigma point transformation.
            ...
            for i = 1:nSp
                measSp(:, i) = tracking.models.MeasurementModel.ctMeasFunc(spPts(:, i));
            end
    ...
    methods (Access = private)
        ...
        % GENERATESIGMAWEIGHTS  Generate sigma points and weights.
            [w, spPts] = ...
                tracking.core.FilterUtils.generateSigmaPoints(state, covar, obj.stateDim, ...
