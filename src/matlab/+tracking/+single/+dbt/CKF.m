classdef CKF < tracking.single.dbt.KalmanFilterBase
...
%
%   See also: tracking.single.dbt.KalmanFilterBase, tracking.single.dbt.EKF, tracking.single.dbt.UKF, tracking.single.dbt.Config

    methods
        function obj = CKF(cfg)
        % CKF  Create Cubature Kalman Filter with configuration.
        %   obj = CKF(cfg) initializes with tracking.single.dbt.Config object.
    ...
    methods (Access = protected)
        ...
        % PREDICTSTEP  CKF prediction using cubature point transformation.
            ...
            for i = 1:nCp
                F = tracking.models.MeasurementModel.ctDynamicMatrix(obj.config.dt, cpPts(:, i));
                preCp(:, i) = F * cpPts(:, i);
            end
        ...
        % UPDATESTEP  CKF update using cubature point transformation.
            ...
            for i = 1:nCp
                measCp(:, i) = tracking.models.MeasurementModel.ctMeasFunc(cpPts(:, i));
            end
            measPre = measCp * wCp';
    ...
    methods (Access = private)
        ...
        % GENERATECUBATUREPOINTS  Generate cubature points and weights.
            [wCp, cpPts] = tracking.core.FilterUtils.generateCubaturePoints(state, covar, obj.stateDim);
        end
    end
