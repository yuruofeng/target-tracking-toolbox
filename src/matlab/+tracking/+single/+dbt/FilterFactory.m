...
%
%   使用方法：
%       filter = tracking.single.dbt.FilterFactory.create('EKF', config);
%       filter = tracking.single.dbt.FilterFactory.create('IMM', immConfig);
%       filter = tracking.single.dbt.FilterFactory.createEKF('CV', dt, q);
%
%   See also: tracking.single.dbt.EKF, tracking.single.dbt.UKF, tracking.single.dbt.CKF, tracking.single.dbt.IMM, tracking.single.dbt.ParticleFilter
    ...
    methods (Static)
        ...
        %       filter - 创建的滤波器对象
            switch upper(filterType)
                case 'EKF'
                    filter = tracking.single.dbt.FilterFactory.createEKFFromConfig(varargin{:});
                    
                case 'UKF'
                    filter = tracking.single.dbt.FilterFactory.createUKF(varargin{:});
                    
                case 'CKF'
                    filter = tracking.single.dbt.FilterFactory.createCKF(varargin{:});
                    
                case 'IMM'
                    filter = tracking.single.dbt.FilterFactory.createIMM(varargin{:});
                    
                case 'PF'
                    filter = tracking.single.dbt.FilterFactory.createPF(varargin{:});
                    
                case {'EKF-CV', 'EKF-CA', 'EKF-CT', 'EKF-SINGER', 'EKF-CS'}
                    filter = tracking.single.dbt.FilterFactory.createMotionModelEKF(filterType, varargin{:});
                    
                otherwise
                    error('dbt:UnsupportedType', ...
                        '不支持的滤波器类型: %s。支持的类型: %s', ...
                        filterType, strjoin(tracking.single.dbt.FilterFactory.SUPPORTED_TYPES, ', '));
            end
        end
        ...
        % CREATEEKFFROMCONFIG  从配置创建EKF。
            if nargin < 1
                config = tracking.single.dbt.Config();
            end
            filter = tracking.single.dbt.EKF(config);
        end
        ...
        % CREATEUKF  创建UKF滤波器。
            ...
            filter = tracking.single.dbt.UKF(config);
        end
        ...
        % CREATECKF  创建CKF滤波器。
            ...
            filter = tracking.single.dbt.CKF(config);
        end
        
        function filter = createIMM(config)
        % CREATEIMM  创建IMM滤波器。
            if nargin < 1
                config = tracking.single.dbt.ConfigIMM();
            end
            filter = tracking.single.dbt.IMM(config);
        end
        ...
        % CREATEPF  创建粒子滤波器。
            ...
            filter = tracking.single.dbt.ParticleFilter(config);
        end
        ...
        %       filter = createMotionModelEKF('EKF-CV', config)
            ...
            if nargin >= 2 && isa(varargin{1}, 'tracking.single.dbt.MotionModelConfig')
                config = varargin{1};
            else
                config = tracking.single.dbt.FilterFactory.parseMotionConfig(modelType, varargin{:});
            end
            
            filter = tracking.single.dbt.MotionModelEKF(config);
        end
        ...
        %       filter - IMM滤波器对象
            if nargin >= 1
                ...
                config = tracking.single.dbt.ConfigIMM('modelTypes', modelTypes);
                filter = tracking.single.dbt.IMM(config);
            else
                filter = tracking.single.dbt.IMM();
            end
        end
        ...
        %       config - MotionModelConfig对象
            ...
            config = tracking.single.dbt.MotionModelConfig(modelType, ...
                'dt', p.Results.dt, ...
                'q', p.Results.q);
        ...
        % GETSUPPORTEDTYPES  获取支持的滤波器类型列表。
            types = tracking.single.dbt.FilterFactory.SUPPORTED_TYPES;
        end
        ...
        % DISPLAYINFO  显示滤波器类型信息。
            if nargin < 1
                fprintf('支持的滤波器类型:\n');
                for i = 1:length(tracking.single.dbt.FilterFactory.SUPPORTED_TYPES)
                    fprintf('  - %s\n', tracking.single.dbt.FilterFactory.SUPPORTED_TYPES{i});
                end
            else
                switch upper(filterType)
                    case 'EKF'
