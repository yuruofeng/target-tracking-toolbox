...
%       tbdAlgo = tracking.single.tbd.DpTbd(cfg);  % 或 tracking.single.tbd.PfTbd(cfg)
%       tbdAlgo.run(measData, psfKernel);
%       [track, score] = tbdAlgo.getResults();
%
%   See also: tracking.single.tbd.DpTbd, tracking.single.tbd.PfTbd, tracking.single.tbd.Config
    ...
    methods
        ...
        % HASVALIDCONFIG  检查配置是否有效。
            isValid = ~isempty(obj.config) && isa(obj.config, 'tracking.single.tbd.Config');
        end
    end
