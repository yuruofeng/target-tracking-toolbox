...
%
%   See also: tracking.metrics.OspaMetric, tracking.multi.rfs.phd.MultiModelFilter
    ...
    methods (Static)
        ...
        %       Cost     - Total cost of minimum matching
            ...
            Edge = P_cond;
            Edge(P_cond ~= Inf) = 0;
            cnum = tracking.association.Hungarian.min_line_cover(Edge);
            ...
            while exit_flag
                switch stepnum
                    case 1
                        [P_cond, stepnum] = tracking.association.Hungarian.step1(P_cond);
                    case 2
                        [r_cov, c_cov, M, stepnum] = tracking.association.Hungarian.step2(P_cond);
                    case 3
                        [c_cov, stepnum] = tracking.association.Hungarian.step3(M, P_size);
                    case 4
                        [M, r_cov, c_cov, Z_r, Z_c, stepnum] = tracking.association.Hungarian.step4(P_cond, r_cov, c_cov, M);
                    case 5
                        [M, r_cov, c_cov, stepnum] = tracking.association.Hungarian.step5(M, Z_r, Z_c, r_cov, c_cov);
                    case 6
                        [P_cond, stepnum] = tracking.association.Hungarian.step6(P_cond, r_cov, c_cov);
                    case 7
                        exit_flag = 0;
                end
            end
    ...
    methods (Static, Access = private)
        ...
        function cnum = min_line_cover(Edge)
            [r_cov, c_cov, M, ~] = tracking.association.Hungarian.step2(Edge);
            [c_cov, ~] = tracking.association.Hungarian.step3(M, length(Edge));
            [~, r_cov, c_cov, ~, ~, ~] = tracking.association.Hungarian.step4(Edge, r_cov, c_cov, M);
