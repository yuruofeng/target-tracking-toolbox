...
%   Tests the correctness of multi-target RFS/PHD package components.
    ...
    methods (TestMethodSetup)
        function setupModel(testCase)
            testCase.Model = tracking.multi.rfs.phd.Config();
        end
    end
    ...
    methods (Test)
        ...
        function testGaussMixturePrune(testCase)
            ...
            [wP, xP, PP] = tracking.multi.rfs.phd.GaussMixture.prune(w, x, P, 0.1);
        ...
        function testMathUtilsExpandState4to6(testCase)
            m4 = [1; 2; 3; 4];
            m6 = tracking.multi.rfs.phd.MathUtils.expandState4to6(m4);
        ...
        function testMathUtilsCoordinateConversion(testCase)
            cart = [1; 1];
            polar = tracking.multi.rfs.phd.MathUtils.cartToPolar(cart);
            cartBack = tracking.multi.rfs.phd.MathUtils.polarToCart(polar);
        ...
        function testCvFilter(testCase)
            ...
            [mPred, PPred] = tracking.multi.rfs.phd.CvFilter.predict(model, m, P);
            testCase.assertEqual(size(mPred, 1), 4, 'Predicted state should be 4D');
            z = [100; 200];
            [qz, mUpd, PUpd] = tracking.multi.rfs.phd.CvFilter.update(z, model, mPred, PPred);
        ...
        function testCtFilter(testCase)
            ...
            m = [100; 10; 200; 20; 0; 0];
            P = repmat(eye(6), [1, 1, 1]);
            [mPred, PPred] = tracking.multi.rfs.phd.CtFilter.predict(model, m, P);
            testCase.assertEqual(size(mPred, 1), 5, 'Predicted state should be 5D');
            xd = [100; 10; 200; 20; model.omega];
            xNew = tracking.multi.rfs.phd.CtFilter.genNewState(model, xd, 'noiseless');
        ...
        function testCaFilter(testCase)
            ...
            m = [100; 10; 0; 200; 20; 0];
            P = repmat(eye(6), [1, 1, 1]);
            [mPred, PPred] = tracking.multi.rfs.phd.CaFilter.predict(model, m, P);
            testCase.assertEqual(size(mPred, 1), 6, 'Predicted state should be 6D');
            z = [100; 200];
            [qz, mUpd, PUpd] = tracking.multi.rfs.phd.CaFilter.update(z, model, mPred, PPred);
        ...
        function testSimmUtils(testCase)
            ...
            [w0, m0, P0, cNorm] = tracking.multi.rfs.phd.SimmUtils.inputInteraction(1, model, est, w, m, P, L, miu);
            testCase.assertEqual(length(w0), 3, 'Should have 3 model inputs');
        end

        function testHungarian(testCase)
            costMatrix = [4 1 3; 2 0 5; 3 2 2];
            [matching, cost] = tracking.association.Hungarian.solve(costMatrix);
            ...
            [matchingInf, costInf] = tracking.association.Hungarian.solve(costMatrixInf);
        ...
        function testOspaMetric(testCase)
            X = [0 10 20; 0 0 0];
            Y = [1 11 21; 0 0 0];
            [dist, locErr, cardErr] = tracking.metrics.OspaMetric.compute(X, Y, 30, 1);
            ...
            [distEmpty, ~, ~] = tracking.metrics.OspaMetric.compute([], [], 30, 1);
            testCase.assertEqual(distEmpty, 0, 'Distance between empty sets should be 0');
            [distOne, ~, cardErrOne] = tracking.metrics.OspaMetric.compute(X, [], 30, 1);
        ...
        function testImmPhdFilter(testCase)
            ...
            meas.Z = scenario.Z;
            meas.K = scenario.K;
            filter = tracking.multi.rfs.phd.ImmPhdFilter(model);
        ...
        function testSimmPhdFilter(testCase)
            ...
            meas.Z = scenario.Z;
            meas.K = scenario.K;
            filter = tracking.multi.rfs.phd.SimmPhdFilter(model);
            est = filter.run(meas, truth);
            testCase.assertEqual(length(est.IMMX), meas.K, 'Should have estimates for all times');
        end
